...
public class ValidationBuilderTests
{
    ...
    public void AddValidator_Invalid_Parameters()
    {
        ...
        Assert.Equal(
            "Type `Limen.Tests.ObjectModelValidator5` must be a non-abstract, non-static class to be registered as a validator. (Parameter 'validatorType')",
            exception.Message);

        var exception2 = Assert.Throws<ArgumentException>(() => builder.AddValidator(typeof(ObjectModel)));
        Assert.Equal(
            "Type `Limen.Tests.ObjectModel` is not a valid validator; it does not derive from `AbstractValidator<T>`. (Parameter 'validatorType')",
            exception2.Message);
    }
