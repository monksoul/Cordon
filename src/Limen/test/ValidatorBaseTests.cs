...
public class ValidatorBaseTests
{
    ...
    public void SetResourceAccessorByPropertyLookup_Invalid_Parameters()
    {
        ...
        Assert.Equal(
            "The resource type `Limen.Tests.TestValidationMessages` does not have an accessible static property named `NotFound`.",
            exception.Message);
        ...
        Assert.Equal(
            "The property `Culture` on resource type `Limen.Tests.TestValidationMessages` is not a string type.",
            exception2.Message);
    }
